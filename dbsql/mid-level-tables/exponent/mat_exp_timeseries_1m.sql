    ...
    LOOP
        ...
        WITH vault_1m AS (
            SELECT
                ...
                LAST(v.start_ts, v.bucket_time)                   AS start_ts,
                LAST(v.duration, v.bucket_time)                    AS duration,
                LAST(v.maturity_ts, v.bucket_time)                 AS maturity_ts,
                LAST(v.total_sy / POWER(10, v_vault.decimals), v.bucket_time)           AS total_sy_in_escrow,
                LAST(v.sy_for_pt / POWER(10, v_vault.decimals), v.bucket_time)          AS sy_for_pt,
                LAST(v.pt_supply / POWER(10, v_vault.decimals), v.bucket_time)          AS pt_supply,
                LAST(v.treasury_sy / POWER(10, v_vault.decimals), v.bucket_time)        AS treasury_sy,
                LAST(v.uncollected_sy / POWER(10, v_vault.decimals), v.bucket_time)     AS uncollected_sy,
                LAST(v.last_seen_sy_exchange_rate, v.bucket_time)  AS last_seen_sy_exchange_rate,
                LAST(v.all_time_high_sy_exchange_rate, v.bucket_time) AS all_time_high_sy_exchange_rate,
                LAST(v.final_sy_exchange_rate, v.bucket_time)      AS final_sy_exchange_rate,
                LAST(v.interest_bps_fee, v.bucket_time)            AS interest_bps_fee,
                LAST(v.min_op_size_strip / POWER(10, v_vault.decimals), v.bucket_time)  AS min_op_size_strip,
                LAST(v.min_op_size_merge / POWER(10, v_vault.decimals), v.bucket_time)  AS min_op_size_merge,
                LAST(v.status, v.bucket_time)                      AS status,
                LAST(v.max_py_supply / POWER(10, v_vault.decimals), v.bucket_time)      AS max_py_supply,
                LAST(v.c_vault_collateralization_ratio, v.bucket_time) AS c_vault_coll_ratio,
                LAST(v.c_vault_uncollected_yield_ratio, v.bucket_time) AS c_vault_uncoll_yield,
                LAST(v.c_vault_treasury_ratio, v.bucket_time)      AS c_vault_treasury,
                LAST(v.c_vault_available_liquidity / POWER(10, v_vault.decimals), v.bucket_time) AS c_vault_avail_liq,
                LAST(v.c_vault_yield_index_health, v.bucket_time)  AS c_vault_yield_health,
                LAST(v.slot, v.bucket_time)                        AS slot
            FROM exponent.cagg_vaults_5s v
            WHERE v.vault_address = v_vault.vault_address
              AND v.bucket_time >= v_seed_from
            GROUP BY time_bucket('1 minute', v.bucket_time)
        ),
        market_1m AS (
            SELECT
                time_bucket('1 minute', m.bucket_time) AS bt,
                LAST(m.pt_balance / POWER(10, v_vault.decimals), m.bucket_time)  AS pt_balance,
                LAST(m.sy_balance / POWER(10, v_vault.decimals), m.bucket_time)  AS sy_balance,
                ...
                LAST(m.lp_escrow_amount / POWER(10, v_vault.decimals), m.bucket_time) AS lp_escrow_amount,
                LAST(m.max_lp_supply / POWER(10, v_vault.decimals), m.bucket_time) AS max_lp_supply,
                ...
                LAST(m.c_pt_base_price, m.bucket_time)             AS pt_base_price
            FROM exponent.cagg_market_twos_5s m
            WHERE m.market_address = v_vault.market_address
              AND m.bucket_time >= v_seed_from
            GROUP BY time_bucket('1 minute', m.bucket_time)
        ),
        yt_1m AS (
            SELECT
                time_bucket('1 minute', y.bucket_time) AS bt,
                LAST(y.yt_balance / POWER(10, v_vault.decimals), y.bucket_time) AS yt_escrow_balance
            FROM exponent.cagg_vault_yt_escrow_5s y
            WHERE y.vault_address = v_vault.vault_address
              AND y.bucket_time >= v_seed_from
            GROUP BY time_bucket('1 minute', y.bucket_time)
        ),
        sy_1m AS (
            SELECT
                time_bucket('1 minute', s.bucket_time) AS bt,
                LAST(s.exchange_rate, s.bucket_time) AS sy_exchange_rate
            FROM exponent.cagg_sy_meta_account_5s s
            WHERE s.mint_sy = v_vault.mint_sy
              AND s.bucket_time >= v_seed_from
            GROUP BY time_bucket('1 minute', s.bucket_time)
        ),
        tx_1m AS (
            SELECT
                time_bucket('1 minute', t.bucket_time) AS bt,
                SUM(t.amm_pt_swap_count)::INTEGER      AS amm_pt_swap_count,
                SUM(t.amm_pt_in)                       AS amm_pt_in,
                SUM(t.amm_pt_out)                      AS amm_pt_out,
                SUM(t.amm_pt_volume)                   AS amm_pt_volume,
                SUM(t.amm_pt_net_flow)                 AS amm_pt_net_flow,
                SUM(t.lp_deposit_count)::INTEGER        AS lp_deposit_count,
                SUM(t.lp_withdraw_count)::INTEGER       AS lp_withdraw_count,
                SUM(t.lp_pt_in)                        AS lp_pt_in,
                SUM(t.lp_pt_out)                       AS lp_pt_out,
                SUM(t.lp_sy_in)                        AS lp_sy_in,
                SUM(t.lp_sy_out)                       AS lp_sy_out,
                SUM(t.lp_tokens_minted)                AS lp_tokens_minted,
                SUM(t.lp_tokens_burned)                AS lp_tokens_burned
        ...
        combined AS (
            SELECT
                ...
                CASE WHEN COALESCE(vlt.total_sy_in_escrow, 0) > 0
                     THEN ROUND(vlt.sy_for_pt / vlt.total_sy_in_escrow * 100, 4) END AS sy_for_pt_pct_sy,
                CASE WHEN COALESCE(vlt.total_sy_in_escrow, 0) > 0
                     THEN ROUND((vlt.total_sy_in_escrow - vlt.sy_for_pt) / vlt.total_sy_in_escrow * 100, 4) END AS sy_yield_pool_pct,
                CASE WHEN (vlt.total_sy_in_escrow - vlt.sy_for_pt) > 0
                     THEN ROUND(vlt.uncollected_sy / (vlt.total_sy_in_escrow - vlt.sy_for_pt) * 100, 4) END AS sy_yield_utilization_pct,
                CASE WHEN vlt.last_seen_sy_exchange_rate IS NOT NULL
                     THEN ROUND((vlt.last_seen_sy_exchange_rate - 1.0) * 100, 6) END AS sy_cumulative_yield_pct,
                CASE WHEN COALESCE(vlt.pt_supply, 0) > 0
                     THEN ROUND((vlt.sy_for_pt * COALESCE(vlt.last_seen_sy_exchange_rate, 1.0) - vlt.pt_supply) / vlt.pt_supply * 100, 4) END AS sy_collateral_buffer_pct,
                ...
                CASE WHEN mkt.pt_base_price IS NOT NULL AND (mkt.sy_balance + mkt.pt_balance * mkt.pt_base_price / COALESCE(NULLIF(sy_meta.sy_exchange_rate, 0), 1.0)) > 0
                     THEN ROUND(mkt.sy_balance / (mkt.sy_balance + mkt.pt_balance * mkt.pt_base_price / COALESCE(NULLIF(sy_meta.sy_exchange_rate, 0), 1.0)) * 100, 2) END AS pool_depth_sy_pct,
                CASE WHEN mkt.pt_base_price IS NOT NULL AND (mkt.sy_balance + mkt.pt_balance * mkt.pt_base_price / COALESCE(NULLIF(sy_meta.sy_exchange_rate, 0), 1.0)) > 0
                     THEN ROUND((mkt.pt_balance * mkt.pt_base_price / COALESCE(NULLIF(sy_meta.sy_exchange_rate, 0), 1.0)) / (mkt.sy_balance + mkt.pt_balance * mkt.pt_base_price / COALESCE(NULLIF(sy_meta.sy_exchange_rate, 0), 1.0)) * 100, 2) END AS pool_depth_pt_pct,
