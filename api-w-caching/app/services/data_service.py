...
class DataService:
    """Composable data access service that does not depend on FastAPI."""

    def __init__(self, sql_adapter: SqlAdapter):
        self.sql = sql_adapter
        self._cache_ttl_seconds = float(os.getenv("API_CACHE_TTL_SECONDS", "30"))
    ...
    def _cache_set(self, key: str, value: Any, ttl_seconds: float | None = None) -> Any:
    ...
    def _cached(self, key: str, loader: Callable[[], Any], ttl_seconds: float | None = None) -> Any:
        ...
        if not is_loader:
            wait_event.wait(timeout=max(self._cache_ttl_seconds, 1.0))
            cached_after_wait = self._cache_get(key)
            if cached_after_wait is not None:
                return cached_after_wait
            return self._cache_set(key, loader(), ttl_seconds=ttl_seconds)

        try:
            value = loader()
            return self._cache_set(key, value, ttl_seconds=ttl_seconds)
        finally:
            with self._cache_lock:
