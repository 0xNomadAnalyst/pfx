"""
EVM Prospect Search — candidate-generation engine.

Fetches token presence data from 6 EVM DeFi protocol APIs, aggregates by
token symbol across Ethereum + major L2s, scores for interestingness, and
exports feature-rich CSVs designed for downstream enrichment.

Run from repo root:
    python pfx/prospect-search/evm/app/main.py

Or from the app/ directory:
    cd pfx/prospect-search/evm/app && python main.py
"""
...
from fetchers import uniswap, curve, balancer, aave, morpho, pendle  # noqa: E402
...
log = logging.getLogger("evm-prospect-search")
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    proto_file_names = {
        "uniswap": "uniswap_pools.csv",
        "curve": "curve_pools.csv",
        "balancer": "balancer_pools.csv",
        "aave": "aave_reserves.csv",
        "morpho": "morpho_markets.csv",
        "pendle": "pendle_markets.csv",
    }
    ...
    print(f"\n{'='*60}")
    print(f"  EVM Prospect Search complete — {len(prospects)} candidate tokens")
    print(f"  Output: {OUTPUT_DIR}")
    print(f"  Top 10 by interestingness:")
    print(f"{'='*60}")
    cols = [
        "token_symbol", "interest_score", "total_tvl_usd",
        "protocol_count", "venue_type_count", "chain_count",
        "symbol_confidence", "shortlist_tier",
    ]
