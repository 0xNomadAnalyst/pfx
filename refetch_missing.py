...
def rebuild_merged(data_dir: Path, label: str):
    """Merge refetched data with existing merged parquet, deduplicate."""
    merged_files = sorted(data_dir.glob('transaction_details_1*.parquet'))
    if not merged_files:
...
if __name__ == '__main__':
    ...
    ORCA_DIR = PROJECT_ROOT / 'dexes' / 'backfill-qa' / 'pfx' / 'parquet' / 'onyc_backfill_orca'
    RAY_DIR = PROJECT_ROOT / 'dexes' / 'backfill-qa' / 'pfx' / 'parquet' / 'onyc_backfill_raydium'
